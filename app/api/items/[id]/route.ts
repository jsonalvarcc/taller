...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error fetching item: " + error.message },
      { status: 500 },
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (formData.has("observacion"))
      data.observacion = formData.get("observacion");
    if (formData.has("ubicacion")) data.ubicacion = formData.get("ubicacion");
    if (formData.has("estado")) data.estado = formData.get("estado");
    if (formData.has("plantillaId"))
      data.plantillaId = parseInt(formData.get("plantillaId") as string);
...
export async function DELETE(
  ...
  try {
    ...
    const existingItem = await itemRepo.findById(id);
    if (!existingItem) {
      return NextResponse.json({ error: "Item not found" }, { status: 404 });
    }
