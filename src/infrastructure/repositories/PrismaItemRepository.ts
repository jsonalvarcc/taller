...
export class PrismaItemRepository implements IItemRepository {
  ...
  async delete(id: number): Promise<void> {
    ...
    // First delete DetalleNovedadPieza (has required relation to Novedad)
    await prisma.detalleNovedadPieza.deleteMany({
      where: { pieza: { itemId: id } },
    });
    // Delete related novedades
    await prisma.novedad.deleteMany({
      where: { itemId: id },
    });
    // Delete related imagenes
    await prisma.imagenItem.deleteMany({
      where: { itemId: id },
    });
    // Delete piezas (this will also cascade delete imagenPieza)
    await prisma.pieza.deleteMany({
      where: { itemId: id },
    });
    // Finally delete the item (use deleteMany to avoid error if already deleted)
